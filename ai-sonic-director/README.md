...
Il prototipo serve a validare flusso e percezione di valore, non la qualità da mastering. L'elenco onesto delle semplificazioni è in [`docs/FONTE_DI_VERITA.md`](docs/FONTE_DI_VERITA.md) §5 — in sintesi: le identità si adattano all'analisi del brano con regole semplici (l'adattività fine è da MVP), l'analisi è di base, niente account né backend, export WAV in tre versioni per destinazione (Master/Streaming/Social; MP3 da MVP).
