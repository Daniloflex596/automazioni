...
- **Export solo WAV:** le versioni per destinazione ci sono (Master/Streaming/Social), ma la codifica MP3 è rimandata all'MVP. ~~Il loudness è misurato in RMS~~ → **risolto con B3 (2026-06-11):** loudness LUFS-like (K-weighting + gating BS.1770-semplificato) con limiter look-ahead; il tetto è sample-peak, non true-peak (manca oversampling — limite dichiarato).
...
- ~~**Il tetto anti-clipping non è un vero limiter**~~ → **risolto con B3 (2026-06-11):** vero limiter look-ahead (~5 ms) con release esponenziale (80 ms) e guadagno linkato tra canali; Streaming/Social raggiungono il target LUFS-like e tengono i picchi sotto il tetto. Limite residuo: sample-peak non true-peak (oversampling non implementato — dichiarato).
...
| 2026-06-11 | **Chiusura dello step B3 — vero limiter + loudness LUFS-like + A/B loudness-matched** (commit `5402db9`). *Perimetro rispettato:* zero dipendenze nuove, zero espansioni laterali. *Implementazione:* `loudness.js` (nuovo): K-weighting stadio 1 (high-shelf +4 dB, coefficienti DeMan/BS.1770) + stadio 2 (high-pass ~38 Hz), `measureLoudness` con gating assoluto (−70) e relativo (−10 LU) su blocchi 400 ms/hop 100 ms; `limitChannels` look-ahead ~5 ms (deque O(n) monotona), release 80 ms, guadagno linkato tra canali, clamp finale. `export.js`: target `targetLufs` al posto di `targetRms`; Streaming/Social raggiungono il target LUFS-like e poi passano dal limiter; Master trasparente (solo protezione picchi). `engine.js`: `setLoudnessTrims` (≤1, mai amplifica), `renderSegment` offline su segmento arbitrario. `studio.js`: `matchSegment` (8 s centrali, misura `originalLufs` all'apertura), `refreshLoudnessMatch` debounced 350 ms su ogni cambio parametri/identità, nota onesta nel passo Confronto, cleanup con `clearTimeout` e invalidazione token. *Test:* +1 controllo (nota parità di volume nel Confronto). *Esito:* **54/54 ✅**, zero errori JS. *Limiti residui dichiarati:* sample-peak ≠ true-peak (manca oversampling — documentato §5); pareggio A/B su segmento, non sull'intero brano (stima, dichiarata nell'UI). *Debito deploy:* B3 non ancora su gh-pages (serve push manuale o allineamento workflow). |
