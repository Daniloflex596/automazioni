...
- **Ultima cosa completata:** **step A2 — validazione robusta dell'upload**: `.opus`/`.oga` e MIME vuoto accettati (decide la decodifica, non l'estensione), file decodificato PRIMA di creare il progetto (zero progetti orfani), rollback del progetto se il salvataggio audio fallisce. Suite e2e riscritta sul caso corrotto + caso vocale `.opus` end-to-end: **40/40 ✅**. A1 + visione + metodo committati in `9ff0fe0`.
- **Prossimo passo:** commit di A2, poi step A3 — gestione errori e stati di attesa.
...
>
> **Step appena chiuso:** A2 — validazione robusta dell'upload (e2e 40/40 ✅). **Prossimo step:** A3 — gestione errori e stati di attesa.
